...
from common.models import Participant, Base, ParticipantGroup, TeamParticipant
...
def save_participants(participants, group_id):
    ...
    try:
        for user in participants:
            ...
            existing_participant_group = db.query(ParticipantGroup).filter_by(
...
def get_participants_by_group(group_id):
    ...
    try:
        ...
        participant_ids = db.query(ParticipantGroup.participant_id).filter_by(group_id=group_id).all()
        participant_ids = [id for (id,) in participant_ids]  # Преобразуем список к обычному списку идентификаторов
...
def get_participants_by_usernames(usernames):
    db = SessionLocal()
    try:
        participants = db.query(Participant).filter(Participant.username.in_(usernames)).all()
...
def get_existing_team_members(team_name, group_id):
    ...
    try:
        ...
        participants = db.query(TeamParticipant).filter_by(team_name=team_name, group_id=group_id).all()
...
def get_teams_by_group(group_id):
    ...
    try:
        ...
        teams = db.query(TeamParticipant.team_name).filter_by(group_id=group_id).distinct().all()
...
def save_team(group_id, team_name, usernames, user_ids):
    ...
    try:
        ...
        db.query(TeamParticipant).filter_by(team_name=team_name, group_id=group_id).delete()
...
def delete_team(chat_id, team_name):
    ...
    try:
        ...
        db.query(TeamParticipant).filter_by(group_id=chat_id, team_name=team_name).delete()
