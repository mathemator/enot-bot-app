...
from utils import check_bot_permissions, send_permission_error_message, create_mentions_text, send_data_not_found_message
...
def handle_all_command(message, bot):
    ...
    message_text = message.text.split(maxsplit=1)[1] if len(message.text.split(maxsplit=1)) > 1 else ""
