# team_service.py
from common.repository import save_team, delete_team, get_teams_by_group, get_existing_team_members, \
    get_participants_by_group
from utils import send_data_not_found_message, check_bot_permissions, send_permission_error_message, \
...
def handle_team_mention(message, bot):
    ...
    for participant in participants:
        if participant.username and participant.username.lower() == command_or_username.lower():
    ...
    if command_or_username in teams:
        ...
        bot.reply_to(message, f"Команда или участник с именем '{command_or_username}' не найдены.")
...
def handle_team(message, bot):
    ...
    if len(parts) < 2:
        send_data_not_found_message(
            message,
            'Пожалуйста, укажите название команды и текст сообщения.',
            bot
        )
        return
    ...
    participants = [p for p in get_participants_by_group(group_id) if p.id in team_member_ids]
...
def handle_teams(message, bot):
    ...
    if not teams:
        send_data_not_found_message(
            message,
            'Ой, похоже, у меня нет данных о командах в этой группе.',
            bot
        )
        return
    ...
    for team in teams:
        ...
        participant_names = ', '.join(f"{p.first_name} {p.last_name or ''}".strip() for p in participants)
